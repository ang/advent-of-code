...
def day(file_name: str):
    ...
    box_to_circuit = connect_boxes(junction_boxes, box_distances)
...
def connect_boxes(junction_boxes: list[JunctionBox], box_distances: list[BoxDistance]):
    box_to_circuit: dict[JunctionBox, Circuit] = {
        box: Circuit(set([box]))
        for box in junction_boxes
    }

    # Keep on connecting until one there's one circuit left. Record the two boxes that made the connection.
    ...
    for box_distance in box_distances:
        ...
        if len(combined_circuit.boxes) == len(junction_boxes):
            answer = box1.x * box2.x
            print("")
            print(f"Last boxes: {box1=} {box2=}")
            print("")
            print(f"{box1.x=} * {box2.x=} = {answer}")
            break
